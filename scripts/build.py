...
<title>Okuto Morikawa - Research Database</title>
<style>
...
.profile-card{border:1px solid #8886;border-radius:12px;padding:1.15rem 1.2rem 1.25rem;margin:.6rem 0 1.2rem}.profile-card .title{font-size:1.35rem}.profile-card .jp{font-size:1.02rem}.profile-card .links{margin-top:.7rem}.profile-card .key{margin-top:.75rem}.profile-contact{font-size:.93rem;margin-top:.32rem}
...
<body>
<h1>Okuto Morikawa - Research Database</h1>
<p class="sub">Public, searchable research database generated from canonical YAML records. Japanese and English fields are retained separately.</p>
<div class="controls">
...
function render(r){
 ...
 if(r.type==='profile'){
   body+=titleBlock(r);let aff=bilingual(r.affiliation_en,r.affiliation_ja);if(aff)body+=`<div class="venue">${esc(aff)}</div>`;if(r.email)body+=`<div class="profile-contact"><a href="mailto:${esc(r.email)}">${esc(r.email)}</a></div>`;const lk=links(r);if(lk)body+=`<div class="links">${lk}</div>`;return `<article class="profile-card" id="${esc(r.id)}"><div class="row"><div class="meta">Profile</div><span class="badge">profile</span></div>${body}<div class="key">${esc(r.id)}</div></article>`;
 }
 if(r.authors)body+=`<div class="authors">${r.authors.map(esc).join(', ')}</div>`;body+=titleBlock(r);
 if(r.type==='presentations'){let ev=bilingual(r.event_en,r.event_ja);if(ev)body+=`<div class="venue">${esc(ev)}</div>`;let detail=`Presenter: ${esc(r.presenter||'')}${r.invited?' · invited':''}`;if(r.slides_file)detail+=` · ${esc(r.slides_file)}`;if(r.date_conflict&&r.researchmap_date)detail+=` · ResearchMap date: ${esc(r.researchmap_date)}`;body+=`<div class="detail">${detail}</div>`}
 else if(r.type==='publications'){let v=[r.journal,r.volume,r.number,r.pages||r.article_number].filter(Boolean).join(' · ');if(v)body+=`<div class="venue">${esc(v)}</div>`}
 else if(r.type==='cv'){let o=bilingual(r.organization_en,r.organization_ja);if(o)body+=`<div class="venue">${esc(o)}</div>`;let e=bilingual(r.event_en,r.event_ja);if(e)body+=`<div class="detail">${esc(e)}</div>`;if(r.kind==='grant'){let pt=bilingual(r.project_title_en,r.project_title_ja);let d=[r.grant_number,pt].filter(Boolean).join(' · ');if(d)body+=`<div class="detail">${esc(d)}</div>`}if(r.kind==='award'){let d=bilingual(r.description_en,r.description_ja);if(d)body+=`<div class="detail">${esc(d)}</div>`;if(r.related_publication){let rp=[r.related_publication.authors?.join(', '),r.related_publication.title,r.related_publication.journal].filter(Boolean).join(' · ');if(rp)body+=`<div class="detail">${esc(rp)}</div>`}}}
 else if(r.type==='software'){let d=bilingual(r.description_en,r.description_ja);if(d)body+=`<div class="detail">${esc(d)}</div>`}
 const lk=links(r);if(lk)body+=`<div class="links">${lk}</div>`;if(r.topics?.length)body+=`<div class="tags">${r.topics.map(t=>`<span class="tag">${esc(t)}</span>`).join('')}</div>`;return `<article id="${esc(r.id)}"><div class="row"><div class="meta">${meta}</div><span class="badge">${esc(r.type)}</span></div>${body}<div class="key">${esc(r.id)}</div></article>`}
...
const p=new URLSearchParams(location.search);q.value=p.get('q')||'';type.value=p.get('type')||'';kind.value=p.has('kind')?p.get('kind'):'profile';lang.value=p.get('lang')||'both';
function update(){let terms=q.value.trim().toLowerCase().split(/\\s+/).filter(Boolean);let rows=data.filter(r=>{if(type.value&&r.type!==type.value)return false;if(kind.value&&r.kind!==kind.value)return false;if(!terms.length)return true;let text=flatten(r).join(' ').toLowerCase();return terms.every(term=>text.includes(term))});count.textContent=`${rows.length} / ${data.length} records`;list.innerHTML=rows.length?rows.map(render).join(''):'<div class="empty">No matching records.</div>';let x=new URLSearchParams();if(q.value)x.set('q',q.value);if(type.value)x.set('type',type.value);x.set('kind',kind.value);if(lang.value!=='both')x.set('lang',lang.value);try{history.replaceState(null,'',location.pathname+(x.toString()?'?'+x:''))}catch(e){}}
